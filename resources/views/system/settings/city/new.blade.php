@php($page = array("5"=>true,"503"=>true,"5031"=>true))
...
<div class="row mt">
  <div class="col-lg-12">
    <div class="form-panel">
      <h4><i class="fa fa-angle-right"></i> Nova Cidade</h4>
      <form class="form-horizontal style-form" method="post">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        @php(
          $fields=array(
            array("Nome","name","Nome da Cidade. Exemplo: Cascavel.","text","",true)
          )
        )
            ...
            <label class="col-sm-12">
              ...
              <select name="country" id="country" class="form-control">
                ...
                @foreach($countries as $country)
                  <option value="{{$country->id}}">{{$country->abbr}}-{{$country->name}}</option>
                @endforeach
              </select>
...
<script type="text/javascript">
  $(document).ready(function () {
    $("#country").change(function(){
      // $.getJSON("{{url("/settings/state/api/listByCountry")}}/"+$(this).val(),function(data){
      //   $("#state").html(data);
      // });

      $.ajax({
        type: "get",
        url: "{{url("/settings/state/api/listByCountry")}}/"+$(this).val(),
        dataType: "html",
