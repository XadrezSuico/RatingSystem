...
<div class="row mt">
  <div class="col-lg-12">
    <div class="form-panel">
      <h4><i class="fa fa-angle-right"></i> Editar País: {{$country->abbr}}-{{$country->name}}</h4>
      <form class="form-horizontal style-form" method="post">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="id" value="{{$country->id}}">
        @php(
          $fields=array(
            array("Nome","name","Nome do País. Exemplo: Brasil.","text",$country->name,true),
            array("Abreviação","abbr","Abreviação do País. Exemplo: BRA.","text",$country->abbr,true)
          )
        )
