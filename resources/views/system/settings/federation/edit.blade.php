...
<div class="row mt">
  <div class="col-lg-12">
    <div class="form-panel">
      <h4><i class="fa fa-angle-right"></i> Editar Federação: {{$federation->abbr}}-{{$federation->name}}</h4>
      <form class="form-horizontal style-form" method="post">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="id" value="{{$federation->id}}">
        @php(
          $fields=array(
            array("Nome","name","Nome do País. Exemplo: Brasil.","text",$federation->name,true),
            array("Abreviação","abbr","Abreviação do País. Exemplo: BRA.","text",$federation->abbr,true)
          )
        )
