...
<div class="row mt">
  <div class="col-lg-12">
    <div class="form-panel">
      <h4><i class="fa fa-angle-right"></i> Editar Estado: {{$state->abbr}}-{{$state->name}}</h4>
      <form class="form-horizontal style-form" method="post">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <input type="hidden" name="id" value="{{ $state->id }}">
        @php(
          $fields=array(
            array("Nome","name","Nome do Estado. Exemplo: Paraná.","text",$state->name,true),
            array("Abreviação/UF","abbr","Abreviação do Estado ou UF. Exemplo: PR.","text",$state->abbr,true)
          )
        )
            ...
            <label class="col-sm-12">
              ...
              <select name="country"  class="form-control">
                @foreach($countries as $country)
                  <option value="{{$country->id}}"@if($country->id == $state->country_id) selected="selected"@endif>{{$country->abbr}}-{{$country->name}}</option>
                @endforeach
              </select>
